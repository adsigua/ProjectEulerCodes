...
lst=[]
f=open("problem_018_data.txt","rt")

for x in f:
    lst.append([int(y) for y in x.split()])

f.close()

tot=[ [[0,0] for y in range(1,x+1)] for x in range(1,15)]
high = len(lst)-1
for i in reversed(range(0,high)):
    h = len(lst[i])
    for j in range(0,h):
        print('{}+{}'.format(lst[i+1][j],lst[i+1][j+1]))
        tot[i][j] = [0,[lst[i+1][j]+lst[i][j]] if lst[i+1][j]>lst[i+1][j+1] else [1,[lst[i+1][j+1]+lst[i][j]]
